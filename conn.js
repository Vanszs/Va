...
const { listmenu2,} = require('./help2')
const { listmenu3,} = require('./store')
const { listmenu4,} = require('./warn')
const { listmenu1,} = require('./owner')
...
const isOwner = ['${setting.ownerNumber}',"6285381568989@s.whatsapp.net","6287712645322@s.whatsapp.net","6281230429936@s.whatsapp.net"].includes(sender) ? true : false
...
const botNumber = conn.user.id.split(':')[1] + '@s.whatsapp.net'
...
// if (chats.length === 0) return;
// data_deposit.data.url_nya = (chats);
// var ytmp3 = await fetchJson(`https://saipulanuar.ga/api/download/ytmp3?url=${data_deposit.data.url_nya}`)
// var xx = ytmp3.result
...
msgFilter.addSpam(sender, orang_spam)
reply('Kamu terdeteksi spam bot tanpa jeda,harap gantian. lakukan perintah setelah 5 detik')
}
...
*_Seputar sewa bot & transaksi_*

${strip}Telpon/Spam blokir 🚫${strip}

_Admin 1 : 6285381568989_
...
case 'ownerm':{
  ...
  const gurbot = '6285381568989@s.whatsapp.net'
  const mark_slebew = '0@s.whatsapp.net'
  var menu_nya =`${listmenu1(sender,prefix,ad,namenya,premnya,usernya,romnya,tanggal,jam,no)}`
...
case 'store':{
  ...
  const mark_slebew = '0@s.whatsapp.net'
  var footer_nya =`*Owner* @${setting.ChatOwner.split('@')[0]}`
  var menu_nya =`${listmenu3(sender,prefix,ad,namenya,premnya,usernya,romnya,tanggal,jam,no)}`
  let btn_menu = [
...
case 'owner':{
  if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
  var ownerNya = setting.ownerNumber.split(',')
  var ownerName = setting.ownerName.split(',')
    sendContact(from, ownerNya[0], ownerName[0],msg)
  }
...
- _Fitur Lebih Buanyak Tentunya_
...
_Total Fitur Mungkin sekitar 100 an_
_Topup & Deposit_`
...
var bg_lexxy = '6285381568989@s.whatsapp.net'
...
const listMessage = {
  ...
  footer: '© created by 𝐖𝐡𝐚𝐭𝐬𝐀𝐩𝐩 𝐛𝐨𝐭',
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by 𝐖𝐡𝐚𝐭𝐬𝐀𝐩𝐩 𝐛𝐨𝐭' })
}
console.log(feta)
}
break
...
case 'ai':
 ...
 reply("Karena anda belum terdaftar, bot akan mendaftarkan anda secara otomatis")
 var res_us = `${makeid(10)}`
var user_name = `#GR${makeid(5)}`
let object_user = {"id": sender, "name": user_name, "seri": res_us, "premium": false }
db_user.push(object_user)
...
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
if (cekUser("id", sender) == "6285645302600@s.whatsapp.net") return reply("Karena kamu rei, kamu gabisa bikin stiker wleeee")
if (isImage || isQuotedImage){
await conn.downloadAndSaveMediaMessage(msg, "image", `./sticker/${sender.split("@")[0]}.jpeg`)
...
reply("fitur sedang dikembangan hehe")
...
case 'bocil': case 'ukhty': case 'santuy': case 'rika': 
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
if (cekUser("premium", sender) == false) return reply(mess.OnlyPrem)
reply(mess.wait)
...
if (cekUser("premium", sender) == false) return reply(mess.OnlyPrem)
reply(mess.wait)
const x77 = JSON.parse(fs.readFileSync(`./function/Random_IMAGE/hijaber.json`)); 
const x76 = x77[Math.floor(Math.random() * (x77.length))]
conn.sendMessage(from, { image :{url:x76.url}, caption:"Done!", mentions:[sender]},{quoted:msg})
break
...
var dev = '6285381568989@s.whatsapp.net'
...
const mark_slebew = '0@s.whatsapp.net'
var footer_nya =`*Owner* @${setting.ChatOwner.split('@')[0]}`
var menu_nya =`${listmenu2(sender,prefix,ad,namenya,premnya,usernya,romnya,tanggal,jam,no)}`
let btn_menu = [
{buttonId: `${prefix}sewabot`, buttonText: { displayText: 'Sewa Bot' }, type: 1 },
{buttonId: `${prefix}owner`, buttonText: { displayText: 'Owner' }, type: 1 },
]
...
case 'full2':{
  ...
  const mark_slebew = '0@s.whatsapp.net'
  var footer_nya =`*Owner* @${setting.ChatOwner.split('@')[0]}`
  var menu_nya =`${listmenu4(sender,prefix,ad,namenya,premnya,usernya,romnya,tanggal,jam,no)}`
  let btn_menu = [
  {buttonId: `${prefix}sewabot`, buttonText: { displayText: 'Sewa Bot' }, type: 1 },
  {buttonId: `${prefix}owner`, buttonText: { displayText: 'Owner' }, type: 1 },
  ]
...
if (!q) return reply(`Syntak Error!\n*Contoh:*\n${prefix+command} 628xxx`)
var num = q+"@s.whatsapp.net"
var dev = '62838168989@s.whatsapp.net'
if (num == dev) return reply('Itu developer gua')
if (num == sender) return reply('itu nomor lu sendiri')
await sleep(3000)
conn.sendMessage(num, {text:'p'}, {quoted:virusnya})
